...
double Array::Arithmetic_mean_absolute_value() const
{
	
	return abs(Sum() / capacity());
}
void Array::Back() throw(bad_alloc)
{
	push_back(Sum());
	push_back(Arithmetic_mean_absolute_value());
}
void Array::Task()
{
	for (int i = 0; i < size(); i++)
		if ((i % 2) == 1)
		{
			elems[i] /= Arithmetic_mean_absolute_value();
		}
}
